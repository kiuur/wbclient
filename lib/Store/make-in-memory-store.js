...
export const makeInMemoryStore = (config) => {
  ...
  const fromJSON = (json) => {
    ...
    for (const jid in json.messages) {
      const list = assertMessageList(jid)
      for (const msg of json.messages[jid]) list.upsert(WAProto.WebMessageInfo.fromObject(msg), "append")
    }
  }
  return {
    chats,
