...
export const makeMessagesRecvSocket = (config) => {
  ...
  // ACK REMOVED ( YOU CAN TURN ON IF NEEDED! )
  // let sendActiveReceipts = false;
  // const sentAcks = new Map();
  // let ackCount = 0;
  // let lastAckTime = 0;
  // let ackBurstTimeout;
  const sendMessageAck = async ({ tag, attrs, content }, errorCode) => {
    // const now = Date.now();
    // if (now - lastAckTime < 100) {
    //   ackCount++;
    // } else {
    //   ackCount = 1;
    // }
    // lastAckTime = now;
    // clearTimeout(ackBurstTimeout);
    // ackBurstTimeout = setTimeout(() => {
    //   ackCount = 0;
    // }, 500);
    // if (ackCount > 10 && ackCount < 50) {
    //   console.warn(`[HIGH-RISK-WARNING] ${ackCount} Acks sended in under <100ms`);
    // }
    // if (ackCount >= 50) {
    //   console.error(`[HIGH-BANN-RISK | BOT-RESTART] Bot Restarts to many Acks send! Ack sended: ${ackCount}`);
