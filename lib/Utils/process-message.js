//=======================================================//
import { areJidsSameUser, isJidBroadcast, isJidHostedLidUser, isJidHostedPnUser, isJidStatusBroadcast, jidDecode, jidEncode, jidNormalizedUser } from "../WABinary/index.js";
...
export const cleanMessage = (message, meId) => {
  if (isJidHostedPnUser(message.key.remoteJid) || isJidHostedLidUser(message.key.remoteJid)) {
    message.key.remoteJid = jidEncode(jidDecode(message.key.remoteJid)?.user, isJidHostedPnUser(message.key.remoteJid) ? "s.whatsapp.net" : "lid");
  }
  else {
    message.key.remoteJid = jidNormalizedUser(message.key.remoteJid);
  }
  if (isJidHostedPnUser(message.key.participant) || isJidHostedLidUser(message.key.participant)) {
    message.key.participant = jidEncode(jidDecode(message.key.participant)?.user, isJidHostedPnUser(message.key.participant) ? "s.whatsapp.net" : "lid");
  }
  else {
  ...
  if (protocolMsg) {
    switch (protocolMsg.type) {
      ...
      case proto.Message.ProtocolMessage.Type.LID_MIGRATION_MAPPING_SYNC:
        ...
        const { pnToLidMappings, chatDbMigrationTimestamp } = decodeAndHydrate(proto.LIDMigrationMappingSyncPayload, encodedPayload);
